...
def load_hubmap_data(labeled_file, unlabeled_file, distance_thres, sample_rate):
    ...
    train_df = train_df.loc[
        np.logical_and(train_df["tissue"] == "CL", train_df["donor"] == "B004")
    ]
    test_df = test_df.loc[
        np.logical_and(test_df["tissue"] == "CL", test_df["donor"] == "B005")
    ]
    train_X = train_df.iloc[
        :, 1:49
    ].values  # node features, indexes depend on specific datasets
