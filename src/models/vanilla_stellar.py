...
from utils import (
    ...
    calculate_entropy_logits,
    calculate_entropy_probs,
)
from datasets.stellar_data import StellarDataloader, make_graph_list_from_anndata
from itertools import cycle
import anndata
...
class VanillaStellarClassifficationHead(nn.Module):
    ...
    def __init__(self, input_dim: int, num_classes: int, temperature: float):
        ...
        self.linear = VanillaStellarNormedLinear(
            input_dim, num_classes, temperature=temperature
        )
...
class VanillaStellarFCNet(nn.Module):
    ...
    def __init__(self, input_dim: int, num_classes: int, temperature: float = 10.0):
        ...
        self.classifier = VanillaStellarClassifficationHead(
            input_dim, num_classes, temperature=temperature
        )
...
class VanillaStellarModel(nn.Module):
    ...
    def __init__(
        self, input_dim: int, hid_dim: int, num_classes: int, temperature: float = 10.0
    ):
        super(VanillaStellarModel, self).__init__()
        self.encoder = VanillaStellarEncoder(input_dim, hid_dim)
        self.fc_net = VanillaStellarClassifficationHead(
            hid_dim, num_classes, temperature=temperature
        )
...
class VanillaStellar:
    ...
    def __init__(self, cfg):
        ...
        self.model = VanillaStellarModel(
            cfg.input_dim, cfg.hid_dim, cfg.num_standard_classes + cfg.num_seed_classes
        ).to(
            self.device
        )  # The model has the standard classes and the seed classes
        self.optimizer = optim.Adam(self.model.parameters(), lr=cfg.lr)
        self.seed_model = VanillaStellarFCNet(
            cfg.input_dim, cfg.num_standard_classes
        ).to(self.device)
        ...
        for epoch in range(epochs):
            ...
            for batch in train_progress_bar:
                ...
                train_progress_bar.set_postfix(
                    {
                        "Loss": train_loss_sum / len(train_progress_bar),
                        "Accuracy": train_acc_sum / len(train_progress_bar),
                    }
                )
            ...
            if val_loader is not None:
                ...
                val_progress_bar = tqdm(
                    val_loader, desc=f"Validation - epoch {epoch}", leave=True
                )
                for batch in val_progress_bar:
                    ...
                    val_progress_bar.set_postfix(
                        {
                            "Loss": val_loss_sum / len(val_progress_bar),
                            "Accuracy": val_acc_sum / len(val_progress_bar),
                        }
                    )
    ...
    def train_main_model(
        ...
        for epoch in range(epochs):
            n: int = 0
            mean_uncert = self.calculate_prediction_uncertainty_on_unlabeled(
                extended_unlabeled_loader
            )
            ...
            train_progress_bar = tqdm(
                zip(labeled_loader, cycle(extended_unlabeled_loader)),
            ...
            for lab_graph_batch, ulab_graph_batch in train_progress_bar:
                ...
                lab_graph_batch, ulab_graph_batch = lab_graph_batch.to(
                    self.device
                ), ulab_graph_batch.to(self.device)
                self.optimizer.zero_grad()
                ...
                cos_dist = torch.matmul(
                    feat_norm, feat_norm.T
                )  # F.cosine_similarity(feat, feat, dim=1)
                ...
                ce_idx = torch.cat(
                    [torch.arange(lab_len), lab_len + ulab_ce_idx], dim=0
                )
                ...
                print(
                    f"bce_loss: {bce_loss}, margin_loss: {margin_loss.item()}, entropy_loss: {entropy_loss.item()}"
                )
                loss = (
                    self.cfg.bce_coeff * bce_loss
                    + self.cfg.margin_coeff * margin_loss
    ...
    def estimate_seeds(
        self, unlabeled_loader: DataLoader, num_seed_class: int
        ...
        for batch in unlabeled_loader:
            ...
            for i, novel_cluster_idx in enumerate(novel_cluster_idxs):
                novel_label_seeds[clusters == novel_cluster_idx] = (
                    largest_standard_class_label + i + 1
                )
        ...
        return DataLoader(
            batch_list, batch_size=unlabeled_loader.batch_size, shuffle=True
        )
    ...
    def calculate_prediction_uncertainty_on_unlabeled(
    ...
    def train_stellar(
        ...
        novel_label_seeds_unlabeled_loader = self.estimate_seeds(
            unlabeled_loader, self.cfg.num_seed_classes
        )
        self.train_main_model(
            labeled_loader, novel_label_seeds_unlabeled_loader, epochs
        )
...
class VanillaStellarReduced(ModelBase):
    def __init__(self, cfg):
        ...
        self.device = torch.device(cfg.device)
        self.model = VanillaStellarModel(
            cfg.input_dim, cfg.hid_dim, cfg.num_classes
        ).to(self.device)
        self.optimizer = optim.Adam(self.model.parameters(), lr=cfg.lr)

    def train(self, data: anndata.AnnData) -> None:
        self.model = VanillaStellarModel(
            self.cfg.input_dim, self.cfg.hid_dim, self.cfg.num_classes
        ).to(self.device)
