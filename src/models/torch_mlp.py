...
class TorchMLP(ModelBase):
    ...
    def _train(self, X_train, y_train, log=False, early_stopping=False):
        ...
        if early_stopping:
            X_train_to_loader, X_val, y_train_to_loader, y_val = train_test_split(
            ...
            train_data_loader = DataLoader(
                list(zip(X_train_to_loader, y_train_to_loader)),
                shuffle=True,
        ...
        for epoch in range(MAX_EPOCHS):
            ...
            for i, (input, targets) in enumerate(train_data_loader):
                ...
                if log and (i + 1) % PRINT_EVERY == 0:
                    ...
                    print(
                        "\r   %dm: epoch %d [%s%s] %d%% train loss = %.10f"
                        % (
                            (time.time() - start) // 60,
                            epoch + 1,
                            "".join("#" * (p // 5)),
